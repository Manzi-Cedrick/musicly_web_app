...
const AlbumTable = () => {
    ...
    return (
        <table className='w-full'>
            ...
            {albumData.map((album: albumInt, i: number) => (
                <tr className="border-b-2 hover:bg-slate-900 text-center hover:cursor-pointer border-gray-500 text-[12px] font-semibold">
                    ...
                    <td className='text-center justify-center'>
                        <button onClick={() => { navigate(`/albums/${i}/update`) }} className='bg-main hover:bg-white hover:text-main duration-500 border-2 border-white opacity-95 mx-4 text-black py-1 px-6 text-[12px] rounded-md'>Update</button>
            ...
            <DeleteData id={"1213123"} showModal={modal} onClose={() => setModal(false)} />
        </table>
    )
}
